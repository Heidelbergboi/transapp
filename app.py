...
from flask import Flask, render_template, request, url_for, stream_with_context, Response, send_from_directory
from dotenv import load_dotenv
from boto3.session import Session
from s3_utils import upload_file, download_to_temp

# ── env & aws ──────────────────────────────────────────────────────────
BASE = Path(__file__).parent.resolve(); load_dotenv(BASE/".env")
AWS = Session(
...
def run_cmd(cmd:list[str]):
    """Yield each stdout line; raise CalledProcessError on non-zero exit."""
...
# ── 1. Presigned POST (no ACL) ─────────────────────────────────────────
@app.route("/sign", methods=["POST"])
def sign():
    fn  = request.json["filename"]
    key = f"full/{uuid.uuid4().hex}{Path(fn).suffix or '.mp4'}"
    s3  = AWS.client("s3")
...
def start_job():
    ...
    JOBS[job_id] = {"link":"", "s3_key":d["s3_key"], "parts":max(2,int(d.get("parts",5)))}
